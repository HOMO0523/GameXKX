...
The desktop HUD no longer scales automatically with monitor work-area size. The persisted 100% and 50% choices now resolve to exactly `1.0` and `0.5` on every tested work area. A 100% HUD is allowed to extend beyond a smaller screen, as approved for this trial.
...
- `95608c8` removes work-area-derived automatic scale and keeps only the manual 100%/50% setting.
...
At 100%, the fixed 1820x941 transparent host is larger than the current 1536x816 work area and can extend past the right or bottom screen edge. The active Win32 Region still follows the visible HUD surfaces, and the 50% setting remains the manual fallback when the complete expanded HUD must fit on a smaller display.
