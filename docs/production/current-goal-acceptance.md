...
status: active
owner: codex
updated_at: 2026-09-08T03:56:28+08:00
source_commit: aea5194
working_tree: runtime balance report, Dev UI, formation and shared deck interactions complete; table balance differences documented
---

> **2026-09-10 天赋升级交互与详细属性提示完成**：升级按钮固定64高、24号江湖体，拖动区不透明深灰；金币到账/消费自动刷新可用状态，显示当前等级及失败原因。冷UBT和25项天赋/属性检查通过；纯2D实机1.032秒五次鼠标点击由0/5升至5/5，按钮、选择和拖动位置保持。详细属性的规则/来源分隔及江湖体已完成两种实际悬停检查。临时会话恢复原天赋，正常挂机新增进度已保存，PIE停止、脏包0。见[天赋交互验收](../design/2026-09-10-talent-upgrade-usability.md)与[属性提示验收](../design/2026-09-09-detailed-attribute-tooltips.md)。

> **2026-09-10 工具交互与精简排版完成**：五工具默认开放，九格按模式停用；宝石九格同品质混种类随机合成并显示统一品质概率。自动放置保留首件品质，空格优先最低完整组、无完整组取最多数量且平局较低品质。材料数量/禁用状态、真实宝石孔即时镶嵌拆卸、原属性/新属性区域直接选择已接入。冷UBT、21项关联检查及最后洗炼选择复测通过；2D PIE14项状态检查与50/75实拍完成，17项存档/设置/地图哈希保持。见[最终验收](2026-09-10-tools-redesign-acceptance.md)。

> **2026-09-10 路线节点美术、信息与实物行旅钱完成（工作区 `codex/ui-visual-optimization`）**：局内节点/地图底与外历练三图统一；选点0.16秒画圈并留圈，已有纵向滚动居中、待选图标轻抖，右侧图例与江湖体关名入场。行旅钱从背包/仓库支付局内购买和刷新，局内不产出；普通箱材料池20%，强化石/洗练砂/行旅钱三种等概率，钱一次10个。局外10个100000金币，单个分解固定2500且不受天赋、不加工具经验。装备总表07C/07D同步。冷UBT、地图货币58/58、库存8/8、真实UI交易与最后排版截图通过；原存档设置未变、临时会话/PIE已恢复。见 [验收记录](2026-09-09-route-map-travel-money-acceptance.md)。

> **2026-09-09 必要音效试接入完成**：14类、22个CC0短音效已导入并接入纯2D核心流程，按钮与工具各共用一条。联合冷UBT及Audio 5/5通过；22段真实混音均与源波形匹配，实机出牌、命中、格挡、倒下、胜利/奖励、工具成功/失败及静音已有证据。Dev状态恢复，14份存档哈希保持，沿用交接时50% HUD；PIE停止、脏包0后已交还地图任务。音色仍属试用，未宣称逐卡法术实操或Cook完成。见[音效记录与许可来源](2026-09-09-essential-sfx-trial-progress.md)。

> **2026-09-09 宝石与元素表现进行中**：用户已通过14类、每类一张图标；14张已导入，140个道具与百分比/机制/来源/品质显示接入已冷编译，部分测试夹具修正待复验。装备总表扩为24页并含逐品质名义/有效值。用户追加白猿冰霜、盘角鹿雷击和现有灼烧招式火焰；冰霜/火焰六帧已重绘并透明整理，元数据及播放链尚在接入。当前错开另一任务的编辑器截图，未宣称完成实机验收。见[当前进度](2026-09-09-gem-attributes-progress.md)。

> **2026-09-10 六章插图全部通过，资源接入完成**：用户已确认61张全部通过；S03-01 v3整姿修正后也再次确认“这版通过”，全部图稿哈希锁定。后续53张场景已导入BC7，逐包卸载重载与源像素SHA1核对全部通过；61张1536×512审阅WebP总计约9.90MB。8位新增配角512×512真透明头像已导入BC7并接入，加上首章共12位；原7位主角/主要NPC来源保持。冷UBT通过，主线对白测试涵盖全部19位说话角色加载，真实主线树可打开。后续5章树/8位头像的隔离实拍与原生回看/关闭全部完成，临时会话和50%缩放已恢复，正常17份存档/设置哈希保持；不宣称Cook完成。见[当前制作进度](2026-09-09-remaining-story-art-progress.md)、[逐图复核](../../SourceArt/UI/StoryNodes/Redraw/visual-review-notes.md)。S00-07为“客栈”，傍山客栈及同脸同房四字店名设定沿用[世界观](../design/2026-09-09-recurring-inn-world-setting.md)。

> **2026-09-10 主线入口气血校验修复**：永久生命天赋使当前气血高于基础字段，旧校验阻断保存与任务打开。统一有效上限后已用真实恢复槽正常加载/保存，库存、装备、天赋和剧情逐项保留；真实鼠标“任务→天台山→关闭→重开”通过。最终冷UBT与18项主线/回血检查通过，相邻25项天赋/属性检查通过；扩展49项中另4项旧路线/目录/内力夹具失败单列，未冒充全绿。同源百草小篓回血错误也已修复并验证。见[原因与证据](2026-09-10-story-entry-talent-health-fix.md)。

> **2026-09-09 主线/对白/天赋视觉与关闭修复**：主线已改为横向图卡；对白使用与背包一致的纸面、924宽纸框和框内半身像，幽白单独保留完整火灵轮廓。天赋按用户最新决定恢复四向布局，保留精简文本并新增右侧“合计属性”，同类加成合并、分组排列。关闭背包首帧跳变已定位到ScaleToFit依赖上一帧面积，改用解析器明确比例，50/75/100开关首帧回归通过。最新冷UBT与119项合并检查证据通过；隔离PIE三轮真实关闭/回看/切页/GC回归通过，合计属性及75%/50%关闭按钮实点确认。见[视觉实施记录](../design/2026-09-09-story-ui-visual-implementation.md)。此前宿主/图片生命周期与2D路线回归见[解耦记录](../design/2026-09-09-story-ui-decoupling.md)。61张新图与六章逐节点实机验收不在已完成范围内。

> **2026-09-09 项目图片BC7资源层**：1101纹理已盘点和加载；391项过大资源使用BC7、原像素保持，卸载重载全部验证。322张WebP派生359MB→38.7MB，游戏保留Texture2D路径。见[进度](2026-09-09-image-optimization-progress.md)。整体加载/界面可靠性仍以上方最新回归为准，未宣称项目全面完成。

> **2026-09-09 六章主线重设计中，出图改为逐张确认**：用户批准当前分支实施61节点系统，但否决首版剧情、插图与排版。最新要求每图先核对高光与构图，确认后只出一张，再问用户问题并修改；不得继续批量生成。S02-02船夫泥手主角修订已获用户确认并替换进项目；采用与篝火一致的四边羽化，详情/缩略图实拍及BC7保存重载通过。下一张S02-03须先核对构图。S00—S04有50节点转折草案，S05仍待改写；界面按上方最新验收推进，天赋最终采用原四向方式。首版规则检查与首章实机流程通过不代表新剧情/61图完成。见[实施进度](2026-09-09-main-story-system-progress.md)。

> **2026-09-08 用户新裁决：任务与教程**：旧马车预览废弃。两个圆形入口分别为“任务”（未来任务系统）与“教程”（全部教学任务及教学关卡），共用左侧仓库区域；不再从这两个按钮进入城镇或马车。马车及停车后专用演员已通过 MCP 从场景删除，五项无外部引用资源与专用代码已清理。新界面、教学和商店仍在实现中，未冷编译与 PIE 验收。此裁决覆盖下文 2026-08-31 马车预览的历史验收要求。

> **2026-09-08 当前特效决定**：用户决定暂停 057 全屏阵赏大招，正式战斗只保留竖向六帧落雷；月白四张卡基础气力统一为 1，落雷改按脚底边界定位。此决定覆盖下面较早的 057 预览与接入记录。详见[当前采用方案](2026-09-08-simple-lightning-final.md)。

> **2026-09-08 057大招重绘预览**：用户选定B02/057并要求我方水平镜像；首版12格选帧不准后，已逐帧复核296帧并完成30主关键＋26过渡共56个定位帧重绘，补回镜头变化、223触地与265–274二次扩散。原间隔和2倍节奏GIF、逐帧PNG对照、方向切换与镜像／时间检查已完成；当前是纸底RGB预览，尚未整理透明游戏序列或接入UE。043已选作冰霜大招参考但未重绘。见[动作参考与进度](../design/2026-09-08-task-ultimate-vfx-references.md)。

> **2026-09-08 状态机制条接入与素材筛选（进行中）**：法术任务固定4／5／3格与实际出牌序号、药方固定编号／开方品质／触发状态已接入UI，冷UBT与7项定向检查通过，真实PIE图标遮罩和小尺寸验收未完成，见[进度](2026-09-08-mechanic-hud-progress.md)。第一包国风／中性普通特效已制作原始动作预览，小型黄色落雷笔刷画风已获认可；第二包选材与057重绘见上条。详见[范围与候选](../design/2026-09-08-battle-vfx-scope.md)。出牌策略与怪物强化保持待做。

> **2026-09-07 战报、墨迹资源条与奖励卡补充完成（工作区）**：半透明圆角战报左置、字号缩放与完整裁边；气血/内力共用墨条，数字江湖体；遗物奖励名称/插图/介绍分区，三张奖励牌放大20%、长说明限定范围自动缩小；修复敌方死亡后遗留重复3P。冷UBT Editor与85项定向检查通过，死亡编号已真实出牌复现并复测，原存档/缩放设置/地图哈希保持。见[本轮验收](2026-09-07-battle-log-resource-reward-acceptance.md)。任务机制图标接入进度见上条，完整法师特写仍待后续设计。

> **2026-09-07 编队与背包共用卡组交互完成（工作区）**：编队三张大角色卡、六张横排候选、明确确认/取消与直接编辑卡组；两个入口共用2列/4列、整页半透明选牌和角色草稿。修复空草稿自动填回、展开返回不重绘，以及HUD设置/倍率重建丢卡组页。冷UBT Editor与Development均通过；相关回归87/88（含2项带警告通过），剩余旧武器槽几何断言；三档实拍、真实鼠标7/8跨入口与NPC取消通过。原进度/设置/地图已恢复，未提交。见[本单元验收](2026-09-07-formation-deck-ui-acceptance.md)。

> **2026-09-07 Dev配装、真实数值报告与战斗显示完成（工作区）**：首页一键推荐整备、统一纸底与ItemSlot，补齐真实战斗逐包统计；2,040场、408配置，0运行错误、0伤害对账差异，HTML含热力图与联动记录。修复选择后及回合边界阶段意图、全部角色内力成长、tooltip截断、战斗资源与按钮排版、挂机图集回退和死亡后重入透明。冷UBT双目标、460项回归、最后11项Travel回归及21怪物42路径实际加载通过；原存档/设置/地图恢复。三表仍有20项宝石数值、135级装备上限及7条表达差异，调整建议尚未写回平衡参数。见[本单元验收](2026-09-07-dev-balance-ui-acceptance.md)。用户追加的编队与共用卡组交互已完成，见上方记录。

> **2026-09-06 F10开发手札完成（工作区）**：用户单独批准继续当前UI分支。水墨面板与同一套运行时JSON命令覆盖物品、独立角色配装、真实关卡战斗、快照和AI批测/A-B；F10保留页签及输入，临时会话保护玩家原档。Editor及Development游戏目标冷编译通过，核心4/4、客户端4项、实机20种子及同种子A/B通过。扩展31项中5项旧版本/夹具失败单列；未生成完整Cook/Stage包。存档/设置恢复，未提交。详见 [验收](2026-09-06-dev-workbench-acceptance.md) 与 [使用说明](../design/dev-workbench.md)。

> **2026-09-06 背包横排选人、卡组与属性完成（工作区，基线f3fe371）**：伙伴/NPC改为固定分类按钮上方一排六张竖卡，红色返回置右下，上下三项标签同宽对齐；卡组用更大双列卡，数量紧邻应用按钮；属性中文名称与分组数值覆盖13名角色；仓库/天赋固定墨迹滑块及其他缺失滚动样式补齐。冷UBT、95项回归中91项通过，4项仍为同一基线错误；三档实拍、真实选人/返回、7/8草稿往返与四条拖动路径通过。追加用户反馈后，选人只保留整块背包的单层深色半透明底，土司首领/周光祖卡面略收小；再次冷UBT、6/6聚焦回归、三档实拍及真实返回通过。存档/设置恢复，本轮未再次提交。详见 [验收记录](2026-09-06-backpack-card-row-attributes-acceptance.md)。

> **2026-09-06 所有桌面Tab统一纸底及Git版本整理**：背包/仓库/编队/天赋/工具/历练/选人共用纸底贴图、九宫格与完整纸边留量；天赋保留淡墨节点分区与原纸色说明区，“升级”单行居中，地图编号与状态移到图标右侧，工具解锁前收起原控件、解锁后原位恢复。最终冷UBT及79项回归完成：75项通过、4项错误与旧基线完全一致。全部Tab和天赋/地图三档缩放已实拍，原存档/设置恢复。版本同时包含之前局内UI、Boss整关结算及所需字体，详见 [本轮验收](2026-09-06-desktop-tab-paper-acceptance.md) 与 [随版本实机证据](evidence/2026-09-06-ui/README.md)。

> **2026-09-06 背包纸底裁边追加修正**：用户复核指出初版仍有直切边；实拍确认1450宽的外层裁剪切掉了原纸底左右和上方毛边。现已删除该外层裁剪，保持缩放基准与控件坐标，内部物品/卡组滚动裁剪保留。冷UBT、4/4受影响检查、50/75/100%重新实拍通过；原存档与设置恢复。见 [纸边修正验收](2026-09-06-backpack-paper-edges-acceptance.md)。

> **2026-09-06 背包选人与纸底统一完成（工作区 `codex/ui-visual-optimization`）**：仅释放背包纸底下沿，主角/伙伴/NPC文本入口居中置于立绘下方，伙伴/NPC每页六张大卡，卡名/等级/右下卡面统一。打开和取消不提前切人，查看不改编队，按角色保留未应用卡组草稿；仓库/历练等纸底统一四角尺度与延展方式。最终冷UBT、聚焦10/10、50/75/100%实拍和真实鼠标草稿7/8往返通过。扩大77项中73项通过、4项旧失配仍单列；100%整组垂直适配、挂机数据及动画建议保留待办。存档/HUD已恢复。见 [背包验收](2026-09-06-backpack-picker-paper-acceptance.md) 与 [整体试玩报告](2026-09-06-full-play-audit.md)。

> **2026-09-06 局内重点UI与Boss整关结算完成（工作区 `codex/ui-visual-optimization`）**：商店/山泉/宝匣/篝火/结算统一纸面水墨；卡名与费用用江湖体并放大，取消商店卡名方形底，恢复完整角色卡图并接入NPC立绘。修复战后发药、桌面惰性遗物栏、移除临时上限后的资源越界和单关路线存档校验。v37凭据先存后展示，确认才回挂机，重开恢复同一结算且不重复发奖。冷UBT、核心96/96、最后UI25/25通过，两条真实路线及50/75/100%窗口已实拍。扩展旧测试和U05/U06/U09仍单列，不宣称全项目全绿。详见 [本轮验收](2026-09-06-in-run-ui-settlement-acceptance.md) 与 [扩展回归](2026-09-06-ui-extended-regression.md)。

> **2026-09-05 怪物阶段、关卡、Tooltip与配队分析完成**：21个怪物已统一为数据驱动阶段，地狱精英／首领二、三阶段和能力牌全部接入；伤害包跨阶段安全截断，v36迁移保留合法战斗状态。27关等级固定5～135，189组挑战／游历编制统一，地狱3-1=125、3-3=135。173张文案重新生成，其中171张与运行时数值管线核对，燃灵还原与后巷脱身继续单列且不进入推荐构筑；游戏Tooltip读取实际角色面板。最新HTML按100级队伍、玄甲／山河等实际套装、125级地狱3-1，模拟抽牌、气力、独立内力、NPC四选三、出牌顺序和逐包阶段伤害。冷UBT通过；怪物17/17、训练41/41、玄甲5/5、山河3/3、数据330/330、Tooltip／敌意图／迁移14/14通过。详见docs/production/2026-09-05-enemy-phase-training-analysis-acceptance.md。后续仅剩两张牌的设计决定及与本轮无关的既有全项目基线问题。

> **2026-09-05 玄甲／山河套装运行时完成**：玄甲2／4／6已接入产甲10%、半甲保留、首次格挡80%攻击追击及攻击失血后的全队40%防御护甲＋逐友方援护；山河2／4／6已接入首张主动地势牌抽1、减1气并给其他友方回2内，以及每回合在普通阵师之后追加当前地势。NPC装备进入战斗快照；平原/山崖改全体敌人，治疗/DOT按等级倍率，村寨/洞窟按触发者防御。v35只规范化旧描述符，不补触发。装备Excel的18档、六条批准文本和两条术语已复读通过；冷UBT、聚焦21/21、装备85/85、相邻108/108、迁移15/15均通过。详见docs/production/2026-09-05-xuanjia-shanhe-runtime-acceptance.md。当时列出的Tooltip、怪物与配队后续项现已由上条完成。
...
> **2026-09-03 对象描述补齐**：用户要求使用单体友方、单体敌方、单体友方/敌方、全体敌方、全体友方，并独占一行加粗，只突出对象名称。已更新全部419个品质版本和36条通用阵赏版本；限定自身、群体扩展及地势/重放范围在正文或核对信息中保留。数值、倍率、费用逐项对比均未改变，本轮只改审阅文档与阅读页，未改游戏代码。生成校验和阅读页脚本语法检查通过；浏览器工具的file URL策略拦截页面读取，未宣称浏览器显示验证通过。
