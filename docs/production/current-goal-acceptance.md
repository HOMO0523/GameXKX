...
working_tree: dirty (preserve user L_Main.umap, unrelated probes and source-art changes; current runtime/test updates are through 1cd93d7, this documentation refresh follows the cold UBT/PIE evidence below)
---
# GameXXK 当前目标(滚动指针)
...
- 本轮新增证据：默认 production loop、headless 和 `all` 脚本门禁分别通过（`Saved/HarnessReports/20260818-053550-ai-production-loop.md`、`20260818-053558-ai-production-loop.md`、`20260818-053604-ai-production-loop.md`；`all` 未启动 UnrealEditor）；Pillow 兼容修复后 asset-contract 为 `54/66`，剩余 12 个失败仍未关闭（`Saved/HarnessReports/20260818-054753-ai-production-loop.md`）；mcp-live 报告 `Saved/HarnessReports/20260818-055050-ai-production-loop.md` 仍非绿，真实 flow/HP HUD/旧 party-deck catalog 证据有失败，生命周期 mock 已由 `7b7ab90` 修复且聚焦测试通过。最新冷 UBT（`-NoHotReload`，结果 `Succeeded`）后，`GameXXK.DesktopTraining.Workbench.SlateBuildContract + LayoutContract + GameXXK.Training` 为 `20/20`、0 failed（`Saved/HarnessReports/20260818-065557-ai-production-loop.md`，Automation 目录 `Saved/Automation/DesktopTrainingAndTravelCooldownFinal-20260818`）；其中 Training 18/18 明确验证游历/局内同 seed/概率、1-1 无箱、普通箱 240 秒、精英/首领高级箱 360 秒、离线补算/待领取账本/读档恢复。`Saved/HarnessReports/20260818-065222-ai-production-loop.md` 的工作台 Slate/Layout 聚焦回归为 2/2；`Saved/HarnessReports/desktop-challenge-visible.png` 证明挑战重建后左仓库、中央 BattleBoard、右只读历练地图仍在同一窗口，`Saved/HarnessReports/desktop-travel-cooldown.png` 证明游历挂机条显示掉箱冷却文案。MasterV2 资源合同 `1/1`（`Saved/Automation/MasterV2NavDiscGreen-20260818-r1/index.json`），覆盖 PanelLarge、ItemSlot、EquipmentSlot、Tab、Route 和五张等比 NavDisc；`GameXXK.MVP.SaveGame` `12/12`（`Saved/HarnessReports/20260818-041625-ai-production-loop.md`，Automation 目录 `SaveGameTravelOfflineV20Green-20260818`）。挑战侧壳只读/导航锁定、设置/关闭分离、仓库分页/排序/卸下仍有前置证据。Phase 0 总门禁仍未通过。
...
- **本轮新增游历滚动 MVP**：`FGameXXKTrainingTravelVisualRuntime` 在 `Walking` 阶段循环平移无缝底图并播放左行主角 atlas，`Combat`/`Defeated` 暂停，完成一轮后回到起点；4 项聚焦 Automation 与冷 UBT 通过。它不改变 1-1 默认通关或挑战/游历规则。
- **2D HUD 迁移分支已接线**：隔离副本 `Content/GameXXK/Maps/L_DesktopTrainingHUD.umap` 复制 `L_QingshanInn` 后仅保留 `PlayerStart`，删除放置场景/灯光/出口/角色 Actor；`Town` 状态在本分支默认指向该副本，进入后自动显示工作台；原城镇和 `L_Main.umap` 保持不动并作为回退路径。迁移证据与限制见 `docs/production/2026-08-18-desktop-training-2d-hud-migration.md`。
...
- 非阻塞测试工具维护:更新 `scripts/gamexxk_real_play_flow_mcp.py` 的旧 `pointer_matches_target` 吸附断言，使其符合 `6668146` 冻结的自由鼠标跟随语义；Phase 0 已把 `--script-tests all` 分成 `headless`、`asset-contract`、`mcp-live` 三类，历史 64/86 仅保留作迁移前对照，当前 headless/all 门禁通过，asset-contract 54/66（剩余 12 项为外部源/旧合同/环境问题），mcp-live 已运行但未通过。
