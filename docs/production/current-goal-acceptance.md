...
status: active
owner: codex
updated_at: 2026-09-16T14:10:00+08:00
source_commit: 02917d62
working_tree: 已恢复全量回归能力（1258项/1088过/170既有失败为基线）；学院13门课构建失败根因已修并按B方案置灰；Shift改Slate修饰键；定向234项新增失败0、净修好3项
---

> **2026-09-16 A0+学院+Shift 批次已完成定向验证**：先修 `GameXXKEnemyManaSiphonTest` 空指针崩溃（原先直接终止自动化进程，全量只能跑完22项），恢复回归能力后取得 **09-16 全量基线：1258 项 / 1088 过 / 170 既有失败**——这是 09-13～09-16 改动首次有全量基线。学院课程的“点击不了”根因是 `BuildLoadout` 借用的全新档受 `bProgressivePartySlots` 槽位门限制，13 门课全部构建失败（`CurrentMapId` 断言失败属级联）；按用户决定改为**方案B**：新增 `EvaluateEligibility` 资格规则，未满足前置的课整行置灰并显示缺失原因，`BeginCourse` 用同一门禁防绕过，借用态不再套用 onboarding 门。Shift 先前是 Windows 物理键轮询且宿主注释自承会漏 key-up，现改为 Slate 修饰键并新增 `RebuildRecovery` 测试锁定“丢失悬停必须清模式”“重建后不得继承旧模式”。冷UBT通过；`GameXXK.UI.CardTooltip` 12/12、学院 6项中4项通过；定向234项**新增失败0**，净修好3项。学院仍有2项目标断言红（法师五牌任务重放，疑为测试自动出牌策略不足以完成，未证实、未掩盖）。**未做真实PIE复核**，学院置灰与Shift的玩家侧验收未完成。见[实现与验证](2026-09-16-a0-academy-shift-batch.md)与[基线证据](2026-09-16-regression-baseline-and-academy-blocker.md)。

> **2026-09-16 整体逻辑交接文档已更新（仅文档）**：[交接包](../handoff/README.md) 分为程序架构与表现逻辑、玩法策划、完整未完成清单。按当前代码区分永久状态、游历、挑战、任务战及奖励事务；保留用户新增的技能点卡牌成长、专属玩法引导、仓库、设置、资源、女主、图鉴和远期联机／UGC／DLC 等未完成项。已有底层代码不等于功能验收。本轮未改功能、资源或存档，未继续试玩。历史临时会话写入原因与当前保存调用链的差异记录为待复核，见程序案第10.1节。

> **2026-09-16 怪物意图层级及灼烧已修复**：放大的活动意图卡使用80层，位于角色/命中特效前；固定预告栏保持原层。实际完成每张怪物意图后结算一次自然灼烧，多段不重复，增益/蓄力可触发，死亡来源跳过；伤害沿既有抗性、扣血与阶段管线进入演出队列。最终冷UBT、27/27定向检查通过，已恢复当前普通1-3第4回合并读取到80层，原HP/灼烧保持。扩展UI的3条静态断言单列，见[修复与验证范围](2026-09-16-enemy-intent-foreground-burn.md)。

> **2026-09-16 任务返回及奖励表现已完成，动效获用户验收**：S00-04描述改为“进入战斗，击败兽群”；任务战返回时恢复当前战后对白/结果页面。任务报酬改用大金币和各类宝箱图标、精确数量；领取成功后向现有余额图标/对应箱子入口飞入。结算页分别显示实际箱子类型，确认仅播放已到账奖励反馈。冷UBT通过，62项相关检查及最终33项受影响复测通过；实机+100000金币、10普通箱、10高级箱与实际入账一致，用户确认动效通过。临时会话已恢复；测试存档写入路径的审计与恢复证据单列，见[实现和验收](2026-09-16-story-return-reward-presentation.md)。

> **2026-09-16 开局通关资格与1-2引导已校正**：开局1-1已算挑战通关，1-2立即可挑战、未挑战通关前不可游历。挑战资格和首通/再战统一读取通关记录，移除额外游历/教学计数门槛。游历1-1通关仅触发伙伴准备及“选1-2→挑战”的推荐引导，默认Unset也能接上；进入挑战才暂停游历。首场战斗不绑定1-1，0次游历直接首战1-2的教学专项通过。最终冷UBT及46/46定向检查通过；原试玩档已恢复，实际进入1-2并暂停游历，未重置。见[规则与证据](2026-09-16-travel-to12-guidance.md)。此前“必须实际挑战通关1-1才开放1-2”的描述由本条覆盖。

> **2026-09-16 首战第二回合续接已修正**：小提示“关闭”改为只收起当前段，读完关闭认可当前说明，不再停用整场教学与配套发牌；实际第二回合优先治疗/护甲，不被遗漏的旧结束回合标记挡住。冷UBT与31/31相关检查通过。独立PIE在开局普通节点3第二回合出现治疗，同一节点记录回血16、产甲7、护甲图标/自动及Complete，未到Boss；原53份玩家文件及上一份试玩8份存档保持。本轮新独立试玩仍供用户操作。见[修正与同场证据](2026-09-16-first-battle-same-fight-followup.md)。

> **2026-09-16 首次普通挑战基础教学已实现**：首次点怪进入真实BattleBoard，教学首回合攻击牌必上手、主角治疗/护甲暂缓，敌方行动后下一手补齐；依次接攻击、气力、结束回合、治疗、护甲、真实护甲图标、自动。以实际效果认可其他牌，缺牌只补本场，永久卡组不变；关闭/快速获胜不阻断战斗。v47新档启用、旧档保持安静。最终冷UBT通过，39项相关检查及最后8项首战复测通过；实机已核对首怪触发、攻击到回合结束及关闭，完整支持牌/状态/自动链由自动化覆盖。原53份玩家文件保持；用户正在操作独立PIE，窗口保留测试档。见[实现与覆盖范围](2026-09-16-first-battle-guidance.md)。不代表局外属性/卡组/伙伴等其余补课链全部完成。

> **2026-09-16 挑战流程修复已完成**：结算按实际1–3人生成与展示，单主角首通1-1正常生成凭据并开放1-2；路线节点的可点击状态与颜色同步更新，修复商店退出后灰着可点；挑战节点完成后保持DesktopTrainingHUD逻辑上下文。最终冷UBT及9/9定向检查通过，真实胜利快照结算、确认后进入1-2、商店进出已复核。玩家当前卡住的Boss胜利已通过正常结算事务补为待确认页面并保存（420金币、24经验、1高级箱），未重打或重置，见[实现与验证](2026-09-16-challenge-flow-repair.md)。

> **2026-09-16 F10一键重置存档已完成**：F10「一键整备」顶部新增按钮，一次点击覆盖当前槽位并复用完整新游戏初始化；回到1级、初始资源、单主角1-1与首箱引导。成功后清除旧试验/战斗恢复点，恢复正常保存；写入失败保留旧状态。冷UBT、29/29定向检查、独立PIE重置及真实文件读回通过。原有53份玩家文件哈希保持。另有3项既有模拟检查的问题单独记录，见[实现与验证](2026-09-16-f10-reset-save.md)。

> **2026-09-16 挂机条击杀掉落表现已实现**：元宝替换为宝石同画风的金色方孔铜钱。真实遭遇奖励跟随对应怪物死亡爆出；金币飞向已有底部 Tab 按钮并缩小消失，不新增常驻金币显示或加钱浮字；宝箱飞入对应入口并轻弹。局部坐标覆盖挂机条及底部 Tab 行，不修改奖励、存档或游历节奏。最终冷 UBT 及 14/14 定向检查通过，原有52份玩家存档哈希保持。见[实现与验证](2026-09-16-travel-loot-presentation.md)。

> **2026-09-16 首关装备门槛已验证**：教学/开箱不暂停1-1游历，单开和批开优先固定掉落箱。裸装100生命/15攻击通过四波普通敌人，在第5波精英战败；穿齐六件基础装158生命/21攻击通过两波精英及Boss，整轮7波完成。基础装备基础属性乘2，并将普通1-1敌人生命比例校准为50%。首次缺装备战败优先逐件引导穿戴，穿齐后手动出发，不反复裸装重试。v46兼容已穿基础装的旧档属性缓存。冷编译、实际游历与按钮流程及相关回归通过；结论限定游历，见[实测与验收](2026-09-16-starter-equipment-gate.md)。

> **2026-09-15 普通宝箱引导修订已实现**：复用普通宝箱入口，指定箱子后台固定掉落，逐个发放；后续箱子到账不主动弹提示，玩家点开才触发。文案不透露课程/发箱规则。强化后引导悬停现有装备详情，关闭后推进；分解了解并关闭即可继续，不强迫分解。合成只引导自动填入，最终操作自选。最新冷UBT及120/120相关检查通过，v45转换v44未开箱权益且不重复发装备。旧试玩已保存，Shipping未更新。见[本批实现与验收](2026-09-15-teaching-chests.md)。

> **2026-09-15 出战槽与主线分步引导本批完成**：1-1单主角；通关后1-2开放200金币伙伴槽天赋；1-3开启主线，完成S00-02认识月白后开放200金币NPC槽天赋，月白首次入队。未开放槽位只显示锁；购买后立即高亮编队图标，再分两步选人入队。允许卸下、保留资产，挑战暂停挂机；v43存档保留合法空槽，旧档保留原有免费开放功能。冷UBT及最终68/68相关检查通过，旧角色图集有2条既有缺失警告；52份玩家文件哈希保持。1-1首次装备教学现由教学箱批次实现，Shipping包未更新。见[实现与验收](2026-09-15-party-slot-onboarding.md)、[阶段流程](../design/2026-09-15-stage-onboarding-operations.md)。下方旧讨论中的“待实施／节点待定／仅文档”已被本条覆盖。

> **2026-09-15 最新决定：挑战期间暂停挂机**：用户最终明确“挑战期间暂停挂机”，取代此前“挑战期间持续游历”的表述；并行运行方案撤回，不再移除现有挑战/游历互斥。挑战结束返回桌面后按既有规则恢复游历，暂停时段不产生或重复补算挂机收入；此前已有收益保留。新档单主角、伙伴/NPC槽逐步解锁且可卸下的决定继续有效；其他候选解锁节点仍待定。本轮仅同步文档，没有继续试玩或修改运行时。见[三层流程第2.1节](../design/2026-09-15-three-layer-player-flow-v2.md)。

> **2026-09-15 出战槽渐进解锁与可空槽已确认，待实施**：用户明确新档只有主角，伙伴槽和NPC槽后续逐步解锁；解锁后可以卸下任一可选成员，并按实际编队进入局内。挂机和局内共用实际出战名单，需支持主角单人、主角＋伙伴、主角＋NPC、完整三人四种组合。卸下不删除角色、装备或个人卡组；旧档保持已有出战与资产。此决定替代此前“始终固定三人、NPC槽必须非空”的设计前提。各角色原有携带份额8/5/3继续作为配置基础，具体解锁事件与单人初期关卡待定；本轮仅完成设计梳理，未修改运行时或宣称验收通过。见[三层流程与规则](../design/2026-09-15-three-layer-player-flow-v2.md)。此前混合挂机和局内的三牌/首领草案已撤回，见[边界纠偏](../design/2026-09-15-idle-inrun-boundary-correction.md)。

> **2026-09-14 局内与挂机左右显示改造完成**：我方在左、敌方在右；角色图像、特写、受击/灰狼扑击、HUD与点击代理同步，敌意图移右上并保留阵位身份，挂机背景向左无缝循环、7/21波从左向右推进。原始美术与战斗/存档规则不变。冷UBT、定向3/3、箭头2/2通过；同范围基线157项为140通过/17失败，最终综合160项为143通过/同17既有失败，新增错误0。浮动PIE真实鼠标三目标及退出回挂机通过，42份玩家存档哈希不变，未更新Shipping包。见[实现与验收](2026-09-14-party-left-battle-layout.md)。

> **2026-09-13 剧情自动播放完成**：对白纸框新增默认关闭的自动按钮；最少5秒，长段按字数和标点留出阅读时间，遇选择/战斗按钮暂停等待操作。已解决后台节流导致计时偏慢的问题；冷UBT、38/38对白主线回归、浮动PIE真实翻页与选择暂停通过，原42份存档哈希保持。现有打包文件尚未更新。见[实现与验收](2026-09-13-dialogue-autoplay.md)。

> **2026-09-13 剧情战斗直达修正版**：按用户确认，读完战前对话后点击“进入X-X战斗”直接进入该剧情专属战斗，胜利后回任务后续对话和奖励，不展示中转任务地图。重复恢复旧等待状态的缓存问题已一并修复。冷Editor/Shipping构建通过；43/43剧情与存档定向回归、实际包30命令/30关/132项F10检查、3次旧等待状态恢复检查通过。最终包`Packaged/GameXXK-2D-Shipping-F10-StoryDirect-20260913.zip`，386.68MiB，包含v42存档及此前字体更新。见[实现及验收](2026-09-12-story-direct-battle.md)。


> **2026-09-12 存档加固第一阶段与Steam素材**：新增GVAS原始字节尾部校验、档案归属与提交顺序，存档v42避免旧v41程序误回退；30秒变更检测保存和正常生命周期尽力保存，保留Dev/教学隔离。最终冷UBT与34/34定向回归通过；实际旧档副本升级v42并在全新进程读回金币133320一致，原42文件哈希不变。用户确定名称“霞客行 / CloudFarer”，11张宣传/库/图标及6张原生1920×1080运行中UI渲染图、两张总览、双语文案及上传清单已交付。未重新生成Shipping包，Steam云同步、多角色UI与完整交易逐项审计仍在后续范围。见[验收及边界](2026-09-12-save-hardening-acceptance.md)。
...
> **2026-09-11 角色教学持续指引修复**：目标完成后继续推荐可出牌、目标或结束回合，动画/布局过渡保留可见区域，遮罩覆盖完整viewport且提示避开意图。冷UBT与新版21项通过，29节使用同一推荐器获胜；主角三节实机连续通关/返回/奖励已保存，Dev退出。其他课程完整实拍仍待覆盖，见[本轮证据](2026-09-11-academy-continuous-guidance.md)。


> **2026-09-11 全屏镂空引导已接入**：按用户最终说明改为整个窗口64%黑色遮罩、当前按钮/功能镂空、白字无局部底框。最终冷编译和21项引导回归通过，中文100%/50%实机及23步人工推进已观察；保留玩家卡组和引导进度、退出Dev并恢复正常挂机。完整总目标未完成，见[本轮证据](2026-09-11-fullscreen-guide-mask.md)。


> **2026-09-11 追风词缀、逐箱报告与引导样式待验收**：用户确认每条固定1%、第3张起逐张递增、同名正常叠加；随机抽牌/回气/减费/计数5项退休，旧装备和待选洗炼定向转换。逐箱结构化收据与半透明深底白字引导已接，真实玩家最新进度已保存至002442备份，冷UBT进行中。装备04H、本地化44规则与项目进度表同步；**未宣称本批通过或总目标完成**。见[词缀设计](../design/2026-09-11-affix-wind-ramp.md)、[最新状态](2026-09-10-ui-guidance-localization-state.json)。

> **独立教程与已批准试玩进度**：13课程29节已完全脱离历练阶段，6项独立Academy及双语恢复合计27/27通过；界面23步、退出离线提示、F10临时入口的前轮行为通过。用户要求保留的背包整理与开箱操作已正常写入正式存档，保留独立恢复槽GameXXK_UserUI_Keep_20260910_234719；后续均以最新玩家进度为准，不恢复早前392730/4箱基线。用户明确其他任务均已完成，本任务不再发送跨任务消息。

> **2026-09-10 21:00 教程与剧情测试入口改造（待验收）**：23步界面入门改为真实操作推进、28节角色机制说明补齐中英并接入实际目标计数；退出分别显示真实离线三类上限；F10新增临时解锁全部关卡/任务。已保护最新试玩并启动冷UBT，未宣称通过。总表与状态同步到[设计与验收](../design/2026-09-10-interactive-ui-onboarding.md)和[完整目标状态](2026-09-10-ui-guidance-localization-state.json)。

> **2026-09-10 设置、仓库与中英逐页审校进行中**：设置共用背包945×533区域，关闭恢复原页；26/26定向通过。97.91秒冷UBT后的DPI虚拟化补偿完成中英50/75/100六组真实鼠标点击及实拍，纸面和底部导航均完整。仓库中英三档实拍已移除底部页数/容量/角色卡冗字，操作为取出/存入、Take/Store；完整范围保留Tooltip。3985文本身份、26规则及对应装备49/卡牌173/怪物21英文名持续同步。用户最新要求透明简化三角、禁止Tab键帽底图，工具名旁独立方形展开按钮已实现，104.30秒冷UBT通过，回归/实拍正在进行。其他Tooltip与页面尚未全部验收。见[本地化总表与规则](../design/2026-09-10-ui-localization/README.md)。

> **2026-09-10 挂机存档追加目标继续执行**：已接原子临时写入、完整性封装、三份轮换恢复、离线时间/领奖及开箱事务等修复；此前32/32定向通过，主线另修首次持久化FText规范化并验证真实战斗磁盘读回。完整周期自动保存、退出保存、精确挂机运行态、槽位关联及全部存档UI仍未完成。用户进度多次继续，旧20/21文件快照均不可恢复；每轮编译前按当前真实玩家保存并分时备份，最新在UI任务目录player-before-central-settings-20260910-170317。见[存档设计](../design/2026-09-10-idle-save-system.md)。

> **2026-09-10 界面、本地化、遗物与讨伐扩展进行中**：本任务获准继续 `codex/ui-visual-optimization`；其他任务结束后的整批备份已上传，远端核对 `004c45fb9e6ceaedf340698bf5cc41391a60f626`。设置移除重置入口、中英基础与桌面F1帮助已有6项定向检查通过；45遗物图和5讨伐/宝箱图已导入并完成重启加载检查。遗物目录现46件（16普通/特殊、20稀有、10史诗），30个高品质效果及延迟动作证据已接入，冷UBT与9项基础回归通过；完整实牌平衡、3-4讨伐玩法、全部动态tooltip/字体shader及实机验收仍待完成。设计总表和状态持续更新，**总目标未完成**。见[完整目标](2026-09-10-ui-guidance-localization-goal.md)、[进度状态](2026-09-10-ui-guidance-localization-state.json)。
