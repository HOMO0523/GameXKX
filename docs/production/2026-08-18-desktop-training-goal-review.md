...
本轮已经把“规则/存档/程序化壳”推进到“挑战可创建真实 CardBattle 会话”的阶段，但离用户要求的完整桌面游戏还有四类硬缺口：

1. **游历仍是规则 façade**：没有实机角色走动、遭遇 Actor、伤害/击杀表现、阵亡暂停、失败后重试和离线/持续挂机结算。
...
|---|---|
| 分支 | `main` |
| 本轮代码提交 | `7881927 feat: add opt-in desktop training runtime bridge` |
...
| 1-1 生命例外 | `BuildEncounterSequence(Stage, false)` 挑战生命 >1；`true` 游历序列为 1 HP，含首领；游历奖励无箱 | 纯规则通过 | 没有真实游历战斗与阵亡触发 |
...
| 游历循环 | 规则层能按 7 遭遇推进、首领结算金币经验、失败开关回当前/前一关，1-1 保持 1-1 | 规则 façade | 无走动、攻击、掉血、击杀、阵亡暂停、离线计时、实际收菜窗口 |
...
3. 游历没有独立的 Runner/Actor/定时器/离线时间模型，不能声称已经实现挂机。
...
| Training rules + bridge | `Saved/HarnessReports/20260818-003445-ai-production-loop.md`；`Saved/Automation/Training-20260818-r8/index.json` | 8/8 PASS；含 `RealCardBattleBridge`、`SaveValidation` |
| DesktopTraining geometry | `Saved/HarnessReports/20260818-002649-ai-production-loop.md`；`Saved/Automation/DesktopTraining--20260818-r2/index.json` | 1/1 PASS |
...
| Cold UBT | `GameXXKEditor Win64 Development -NoHotReload -MaxParallelActions=4`，本轮变更后成功 | PASS；未用 Live Coding/Hot Reload |
...
| PIE/MCP 工作台 | 本轮无新的工作台截图/点击流 | 未运行 |
...
1. 先完成 TravelRunner：走动、一次一只怪、普通/次级精英/首领、自动攻击、掉血、击杀、单场结算、阵亡暂停、重试和 1-1 回退。
...
2. 回滚提交 `7881927` 可移除 Training 运行时和 v18 接线，不触碰 `L_Main.umap`；
