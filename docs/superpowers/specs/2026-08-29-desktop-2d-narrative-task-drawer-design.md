...
**日期：** 2026-08-29  
**状态：** 用户逐节确认；2026-08-30 纠正 Narrative 与挂机条所有权边界
...
2. Overlay 按挂机条当前自身状态恢复桌面 HUD 原窗口范围、透明命中区域和位置。
3. WorkbenchLayer 重新显示。
4. 关闭 Workbench Tab、任务抽屉和页面级模态，但不修改挂机条自身折叠状态。
5. Tab 解锁；挂机条在剧情前为展开时仍展开，在剧情前由玩家折叠时仍折叠。
...
8. 按挂机条未被剧情修改的当前状态恢复 Overlay 原窗口范围和命中区域。
9. 显示挂机条、关闭 Workbench Tab 并解锁 Tab；不得调用挂机条折叠动作。
10. 保存“下次从本段入口播放”。
...
- 重复暂停幂等。
- 同一运行会话内，进入剧情前后的 `bIdleStripFolded` 必须严格相等；正常完成、主动暂停、资源失败、Coordinator 失败与待重播恢复均适用。
- `Tab/I/Q/C` 在剧情退出后只改变 Workbench 页面状态，不得顺带展开或折叠挂机条。
...
- 启动后挂机条遵循其普通启动策略；Narrative 状态不得强制改变其折叠状态。Narrative Layer 和任务抽屉均关闭，Tab 可用。
...
- 暂停或正常结束后输入恢复，Workbench Tab 关闭；挂机条自身折叠状态与剧情前一致。
...
2. Narrative Layer 正常结束，Workbench Tab 关闭并解锁，挂机条自身状态保持不变。
