...
每个可交互 NPC 还拥有一个以自身为中心、半径 300 Unreal Units 的圆形交互触发区。触发区只进行查询并仅响应玩家 Pawn 的 Overlap，不阻挡角色、NPC、镜头、攻击或其他碰撞。玩家进入时把该 NPC 登记到角色交互组件的候选集合，离开时立即移除；Overlap 是范围判定的唯一权威来源，候选登记后不再按两 Actor 中心距离重复过滤。按 `F` 时只从当前 Overlap 候选中选择，不扫描全场，也不检查主角朝向。
...
> **已被取代（2026-09-11）**：本节的两列品质表不再有效。现行批准表见
> `docs/design/2026-09-10-hunt-stage-design/GameXXK_讨伐与宝箱设计总表_2026-09-10.xlsx` 的
> `02_宝箱品质概率` 页与 `docs/design/2026-09-10-hunt-stage-design/runtime-contract.md`。
> 现行为基点域四列（普通箱 / 高级箱 / 讨伐箱（普通·困难）/ 讨伐箱（地狱）），每列精确 10000，
> 珍稀及以上严格 1:4:16，普通箱止于至宝、高级箱止于超凡，**宇宙只从地狱讨伐箱直出 0.20%**。
> 本节原表从未接入代码，保留供历史对照。
...
> **已被取代（2026-09-11）**：超凡/天界/登神三档已按用户选的路线修订，现行表见
> `docs/design/2026-09-10-hunt-stage-design/GameXXK_讨伐与宝箱设计总表_2026-09-10.xlsx` 的
> `04_九合一品质表` 页与 `Source/GameXXK/Private/GameXXKToolCombineProbability.cpp`。
> 改动：超凡/天界（原 7、8 档）不再有“原品质”空转结果，改为 90% 进一档 / 10% 进两档；
> 登神（9 档）由 75% 保持 / 25% 宇宙改为 50% 保持 / 50% 宇宙。1～6 档不变。

每次合成都消耗九件同大类（装备或宝石）、同品质输入并生成一件结果。按2026-09-09用户确认，宝石九格各放一颗，允许不同种类混合，产出种类从当前宝石目录等权随机抽取；品质使用下表。即使结果保持原品质，也消耗全部九件。
