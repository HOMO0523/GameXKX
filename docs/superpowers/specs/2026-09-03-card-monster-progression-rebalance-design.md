...
The user confirmed level-one base Mana plus explicitly authored fixed additions: Sorcerer partner 34 and Hero 30 before such additions. Ordinary character-level/star growth and equipment growth must not inflate those pools. Mana itself does not use Defense, card quality, or the DOT level multiplier. This resource prerequisite is implemented at 2e0db81 with cold UBT and 316/316 regression checks; see docs/production/2026-09-03-fixed-mana-equipment-acceptance.md. The separate Ice card revisions still require implementation and certification.

The subsequent user clarification is absolute for equipment: equipment cannot increase Mana for any character. Item base stats, item-level growth, enhancement, fixed legacy item bonuses and percentage MaxMana affixes contribute zero. Remove MaxMana from newly generated/reforged affix pools and from effective equipment-stat displays, while retaining old IDs/snapshots only for save compatibility. Do not invent replacement attributes or reroll owned equipment. The character-level freeze above remains scoped to Hero/Sorcerer; this equipment prohibition applies to all professions and owned NPCs.
...
- `HanXu`: cost 1/Mana 0; retain secondary Armor 40%; gain Mana 6; Mana overflow converts to Armor at 100%. The Energy increase does not promote its separately authored Armor to the 80% primary tier.
...
The confirmed Hero Sorcerer Energy costs are YanXu/HanXu/LeiXu/GuiXu = 1/1/1/0, with unchanged Mana costs 3/0/3/0. Four active cards require 3 Energy before rewards; replays are free. See `docs/superpowers/specs/2026-09-03-sorcerer-energy-cost-design.md`.
...
**Confirmed Energy revision:** only `Profession.Sorcerer.JuLing` (周天归元) and `Profession.Sorcerer.LieFu` (照见五蕴) cost 0 Energy; the other 16 active partner Sorcerer cards cost 1 at every legal quality. Mana, explicit effects and starter refunds retain their values. This supersedes the old all-zero Universal rule. Five distinct cards require 3-5 Energy before rewards; a 4-Energy sequence that later returns 1 still needs 4 before completion. Replays remain free. See `docs/superpowers/specs/2026-09-03-sorcerer-energy-cost-design.md` for the 22-card table and payment boundaries. Older same-round damage fixtures need enough Energy at the new prices; historical Energy-spent fields are not current acceptance values.
...
| `Profession.Sorcerer.FenMaiFu` / 玄冰拓脉 | Common; 1 Energy / 0 Mana | Increase this battle's MaxMana by 4 without changing current Mana; then recover `ceil(current Mana * 10%)` and convert actual overflow. | Increase this battle's MaxMana by another 8 and fill current Mana to that new maximum. |
| `Profession.Sorcerer.LingYanLianDan` / 霜镜叠甲 | Rare; 1 Energy / 0 Mana | If current Armor is zero, recover `ceil(current Mana * 10%)` and convert actual overflow. Otherwise double current Armor, without recovering Mana or applying another multiplier. | Every unique living ally, including the caster, receives `floor(ArmorConsumedByThisIceBlast / 4)` Armor. |
