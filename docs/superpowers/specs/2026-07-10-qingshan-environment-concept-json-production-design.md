...
用户审核 `REF_QS_ENV_STYLE_LOCK` v002 后确认画面仍然过碎，因此 `QS_InkToon_v1`
以及后续全部可生成资产必须写入并遵守 `detail_budget`：

- 单物件必须能在 128 px 缩略图中读清。
- 最多 3 个主形体，最多 5 条或 5 组次级内部线。
- 只使用 2 个主明度和 1 个阴影强调。
- 禁止逐片瓦、重复窗格、逐叶刻画、石块或鹅卵石镶嵌以及细小道具堆叠。
- 建筑屋顶是一个大形加一条屋脊；每面墙最多一门一窗。
- 植物是 2 至 3 个冠团加主干；桥只保留桥面、拱和栏三个大形。
- 山体是一个外轮廓加最多两笔干刷强调；岩石是外轮廓加单一阴影带。
- 地表、水面和道路使用大块平涂，不得加入鹅卵石拼花、碎石噪点或微型纹理。

该预算必须同时存在于 Style Profile、每项资产 JSON 以及未来实际生成提示中。
`REF_QS_ENV_STYLE_LOCK` v003 是当前审批预算内的最终风格板版本；禁止 v004。
