...
Require exact API names, existing-generator calls, `LandscapeEditLayer.h`, `LandscapeEditTypes.h`, `Json`, B1 graph-root acceptance, and rejection of sibling/broad roots. Require source guards for the exact B1 map, locked layer, incompatible landscape, missing material, unowned matching networks, repeated reset, exactly three generated width groups, and stable owned edge labels/digest. Explicitly forbid removed `SetCanHaveLayersContent` calls and forbid implementing the road layer as `ClearEditLayer` followed by the plugin's absolute-height `ApplyRoadInfluenceDualFromProcMeshes`. Require an explicit advanced PCG base seed, edge-exclusion labels/digest, clearance, and material override contract in the public editor header. The true two-run live behavior test is deferred to Task 5 because the B1 map does not exist during Task 3.
...
Use `UQuick_RoadAutomationLibrary : UBlueprintFunctionLibrary`. `EnsureLandscapeInfrastructure` must accept actor label, topology, anchor-transformed desired grid centre/scale, edit-layer name, material object path, and absolute heightmap file path. It computes the actor origin from the grid half-extent instead of assigning the desired centre directly. `GenerateRoadNetwork` accepts the B1 road material path, overwrites `UQuick_RoadRoadSplineWidthComponent::SetHalfWidthCm` on every input spline, rejects unowned matching networks, and applies the material to ribbon output. `RebuildRoadIntersections` must receive both intersection parameters and ribbon parameters (`sample distance`, `width subdivisions`, `curvature threshold`, `max curvature subdivisions`), reject unowned matching networks, and reapply the material to road/intersection components. `ClearAndApplyAllRoadInfluence` must accept the exact Landscape label and implement the Additive delta algorithm: neutralize the road layer, read base height, query `FQuick_RoadRoadMeshInfluenceField`, calculate and smooth desired final height, convert to `DesiredFinal - BaseHeight`, then encode/write that delta only. `ExtractRoadEdges` returns stable labels/count/digest. `AuditInfrastructure` returns network count, source tag/width/material records, triangles, intersection patches, edge splines, edit-layer state, non-neutral delta count, and a digest of the target edit-layer delta buffer.

- [ ] **Step 4: Implement landscape creation and strict rollback**

Before calling `FQuick_RoadLandscapeCreator::TryCreateLandscape`, snapshot all `ALandscape*`. After the call, whether it returned true or false, diff the snapshot; unless success produced exactly one new actor, destroy every newly discovered Landscape and fail. Compute `ActorOriginWorld = DesiredGridCenterWorld + RotationScaleTransform.TransformVector([-(ResolutionX-1)/2, -(ResolutionY-1)/2, 0])`, apply requested label/origin/rotation/scale/material, and call `PostEditChange`. On reuse, require exact topology and compare the reconstructed grid centre; correct or safely repair origin/rotation/scale/material. Include `LandscapeEditLayer.h` and `LandscapeEditTypes.h`; use `GetEditLayerConst`/`CreateLayer`, reject a locked layer, and set it active without legacy layer-enable APIs. Use UE JSON serializers and add `Json` to `Quick_Road.Build.cs`.

- [ ] **Step 5: Implement repeatable road operations**

`ResetRoadInfrastructure` operates only while the current map is under `/Game/GameXXK/Maps/Dev/L_Qingshan_PCG_Dress_B1`. It removes only actors carrying `QingshanB1QuickRoadOwned` plus an allowlisted QuickRoad network/edge/bake category tag. Generate each road tag with the requested width, ribbon settings, and earth-road material; tag networks as owned. Before plugin generation or consolidation, reject any tag-matching network that is not B1-owned. Before and after edge extraction snapshot edge actors, require exactly one new edge actor, assign stable label `QS_B1_QR_Edge_000` and the ownership/category tags, and reject any unowned/duplicate result. Consolidate the three tags with the same ribbon settings. All plugin calls that can mutate selection use an RAII selection restore. Before road influence:

```cpp
Landscape->ClearEditLayer(
    RoadLayer->GetGuid(), nullptr,
    ELandscapeToolTargetTypeFlags::Heightmap, true);
// Read the neutral-layer base, compute DesiredFinal, then write
// AdditiveDelta = DesiredFinal - BaseHeight into RoadLayer.
```

Select the exact Landscape, gather the unique owned consolidated network procedural meshes, apply/smooth through the dedicated delta implementation, extract road edges, and return measured JSON. Do not call the existing absolute-height road influence/smooth functions for the Landscape. Bake must preserve and restore editor selection and select only the unique owned network.

- [ ] **Step 6: Extend only the exact B1 PCG root**

Add `/Game/GameXXK/Environment/TownPCG/B1/` as a third exact managed graph root. Refactor the root safety tests so `VerticalSlice`, `B0R`, and `B1` are accepted while `/Game/GameXXK/Environment/TownPCG/`, sibling prefixes, source maps, and non-Dev maps remain rejected. Preserve the existing three-argument functions exactly and add uniquely named UFUNCTIONs `CreateOrUpdateTownPCGGraphAdvanced` and `AttachTownPCGGraphAdvanced`; do not use reflected overloads. Advanced graph authoring accepts an explicit base seed, stable transform-derived point seeds, sorted road-edge actor labels plus geometry digest and minimum clearance, and material override paths using `FPCGMeshSelectorWeightedEntry::Descriptor.OverrideMaterials`; it persists the compact contract JSON in `Graph->Description`. Advanced attach sets and audits the `UPCGComponent` seed. Existing B0R callers retain `Index + 1` point seeds and no override/description changes.
