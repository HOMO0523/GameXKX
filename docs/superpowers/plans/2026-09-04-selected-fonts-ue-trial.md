...
**Goal:** Import the approved Chinese and English display fonts plus the required glyph fallback into UE 5.8 as isolated trial Font assets, open them in the Font Editor, and collect deterministic import evidence without changing existing UI or maps.
...
- [x] **Step 1: Verify the three source files against the approved hashes**
...
Create a Python entry point with three fixed records: `zh-Hans` → `ZiKuJiangHuGuFeng.ttf`, `en` → `Rancho-Regular.ttf`, and fallback → `SourceHanSansCN-Regular.otf`. The selected display records create companion Runtime Fonts; the fallback record creates only a Font Face. For each record it must:
...
8. configure both Runtime Fonts' fallback typefaces to the imported Source Han Sans face through `ToolsetLibrary`, verify each resolved object path, and save them;
9. close any stale editor instances and reopen both companion Runtime Font assets through `AssetEditorSubsystem` after all validations pass.
...
Expected: `ue-import-result.json` records `ok=true`, exactly three Font Face assets, exactly two Runtime Font assets, all five package saves as successful, and both `fallbacks` entries point to the Source Han Sans face.
...
Require all five `.uasset` files to exist and have non-zero size. The importer result must report matching expected classes and source filenames. Recompute the three source hashes and compare them with the manifest.

- [x] **Step 2: Inspect both Runtime Font assets in the UE Font Editor**

Leave both trial Runtime Font assets open in UE. Confirm the Font Editor renders the Chinese and English faces without a load failure or missing-resource error. In the Chinese sample, verify that `−`, `×`, and `→` resolve through the Source Han Sans fallback instead of rendering missing-glyph boxes. This is a Font Editor trial; it does not claim that existing UMG widgets have been changed.
...
Expected: only the importer and five new trial `.uasset` files are visible under `Content`; `Saved` evidence remains Git-ignored; no existing UI asset appears as modified by this trial.
