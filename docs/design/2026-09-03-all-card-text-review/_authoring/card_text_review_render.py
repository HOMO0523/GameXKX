...
for c in CARDS:
    ...
    for v in c['variants']+c['branches']:
        ...
        assert set(names)<=set(c['pills'])
        assert not set(names)&set(TARGET_LABELS)
...
def card_md(c):
    ...
    if c['common_rules']:
        z+=['','右键说明使用的共同机制，不追加到Shift详述：','']+[f'- {t}' for t in c['common_rules']]
