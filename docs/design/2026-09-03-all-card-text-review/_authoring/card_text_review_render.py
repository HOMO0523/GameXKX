...
readme.insert(10,'- 字体层级：卡名22号并加重，以普通白／稀有蓝／史诗紫区分当前品质；对象名14号加粗，正文13号；游戏共享说明框的交互与排版已接入，逐卡数值正文仍在后续实装范围。')
...
$('count').textContent=`${list.length}／173张`;$('content').innerHTML=list.map(c=>{const vs=q==='native'?[c.variants[0]]:q==='all'?c.variants:c.variants.filter(v=>v.quality===q);return `<article class="${c.status==='文案待审'?'':'candidate'}"><h2 class="quality-name" data-quality="${esc(vs.length===1?vs[0].quality:c.base_quality)}">${esc(c.name)}</h2><div class="meta">${esc(c.group)} · 原生${esc(c.base_quality)} · ${esc(c.status)}</div>`+vs.map(v=>`<section class="variant"><div class="cost">${esc(v.quality)} · ${esc(v.cost)}</div>${pair(v.compact,v.detail,v.target_heading)}${interactivePreview(c,v)}${c.branches.length?'<details><summary>该品质的全部阵赏分支</summary>'+c.branches.filter(b=>b.quality===v.quality).map(b=>`<div class="cost">${esc(b.branch)}阵赏</div>${pair(b.compact,b.detail,b.target_heading)}`).join('')+'</details>':''}</section>`).join('')+(c.common_rules.length?`<details><summary>机制核对资料</summary>${c.common_rules.map(t=>`<div class="rule">${copy(t)}</div>`).join('')}</details>`:'')+`<details><summary>取值与语义核对</summary><code>${esc(c.id)}</code><p>目标入口：${esc(c.target)}</p><p>${esc(c.source_rule)}</p>${c.note?`<div class="review-note">${esc(c.note)}</div>`:''}</details></article>`;}).join('')||'<p>没有匹配卡牌。</p>';}
