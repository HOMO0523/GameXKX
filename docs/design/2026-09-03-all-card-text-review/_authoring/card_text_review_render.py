...
for card in CARDS:
    ...
    card['implementation_note']='文案与运行时数值管线已通过173张卡证书。' if card['status']=='已核对' else '本牌仍有明确列出的设计待确认项。'
...
meta=dict(status='171张已与运行时数值管线核对；2张保留明确设计待确认项；游戏tooltip已接入实际角色面板预览',card_count=173,quality_variant_count=419,universal_reward_variant_count=36,example=dict(attack=100,defense=100,team_max_level=100,party_living_count=3,armor=0,medicine=0,charge=0,momentum=0,enemy_dot=0,enemy_mark=0,hero_mana='30/30',sorcerer_mana='34/34'),sources=['Source/GameXXK/Private/GameXXKCardCatalog.cpp','Source/GameXXK/Private/GameXXKCardQualityRules.cpp','Source/GameXXK/Private/GameXXKCardRules.cpp','docs/superpowers/specs/2026-09-03-card-monster-progression-rebalance-design.md','docs/design/2026-09-03-sorcerer-card-quick-reference.md'],pending=[c['id'] for c in CARDS if c['status']!='已核对'])
...
meta['tooltip_typography']=dict(card_name='22px heavy',target_line='14px bold',body='13px',rarity_colors={'普通':'white','稀有':'blue','史诗':'purple'})
meta['tooltip_interaction']=dict(default='compact',shift='hold_detail',control_key='press_toggle_pills',keep_pills_on_keyup=True,ignore_key_repeat=True,pill_scope='current_card_quality_and_active_branch')
...
def card_md(c):
    ...
    z+=['','[本牌各品质的Ctrlpill说明](11-card-pill-descriptions.md#'+c['name']+')。']
...
shared=['# Pill、状态与交互提示完整文案','', '[返回总表](README.md)','', '共享说明已接入Tooltip。下表“示例／动态字段”不是固定写死的游戏数值；实际界面从当前状态取值。','', '## Pill与状态说明','', '| 名称 | 类别 | 说明文本 | 示例／动态字段 |','|---|---|---|---|']
...
readme=['# 全部卡牌描述文本审阅稿','', '**173张卡、419个合法品质版本、36条通用阵赏品质分支，以及全部共享pill和状态提示。** 171张卡已与运行时数值管线核对，2张保留明确待确认项；tooltip与pill交互已接入。','', '[打开可搜索的文案阅读页](review.html) · [数据全文](card-texts.json) · [共享提示](10-shared-tooltip-text.md)','', '## 本次采用的表达','', '- 攻击简述写计算后的数字，例如“造成120点伤害。”；攻击详述写“造成120%的攻击伤害。”，不追加出手对象、当前攻击值或算式。', '- 取值角色保留在内部核对字段，不塞进攻击详述。目标、群体范围、条件、次数和先后顺序保留。', '- 护甲等简述显示产出数字。详述用自然句说明比例或系数、实际生效的品质与等级加成；没有加成时省略。', '- 每张牌的基础、条件、冲锋／收招、重箭、药方、阵赏均列出。公共机制放在独立说明中，同一术语解释一次。', '- 分支未定的通用阵赏列出全部分支；分支确定后只显示已锁分支。尚未演算完整任务重放时，不伪造未来护甲与阵赏总伤害。', '- 怪物的防御、状态修正、护甲吸收、最终损血由鼠标移到怪物后的tooltip负责。','', '## 数值示例基准','', '每张卡独立计算，示例攻击100、防御100、队伍最高等级100、3名存活友方。初始护甲、药效、蓄力、气势、敌方标记及持续伤害均为0；主角内力30/30、伙伴法师34/34。只计算该效果的自身产出，不把怪物防御、附加持续伤害及反应链合并为攻击数字。','', '这是统一参考示例；实际UI已从同一条运行时数值管线读取角色面板并实时填数；当前属性或卡牌条件改变后，简述数值也改变。多段攻击逐段显示；未来条件奖励保留规则关系。有关血势、药效先获得后治疗、内力先付费后溢出等特殊顺序，已在对应卡下写出。','', '品质按原生及可升级品质逐个列出。所有治疗系数都是基础值。25点治疗在百级稀有使用600%增幅，零药效为150、药效5时为180；史诗700%增幅则为175／210。等级与品质统一合入最终增幅倍率，600%表示6倍。复制、护甲翻倍及返还不再次缩放。','', '## 分组文本表','', '| 分组 | 卡数 | 品质版本 | 完整表 |','|---|---:|---:|---|']
...
const list=DATA.cards.filter(c=>(!g||c.group===g)&&(!s||JSON.stringify(c).toLowerCase().includes(s))&&(!$('pending').checked||c.status!=='已核对')&&(q==='native'||q==='all'||c.variants.some(v=>v.quality===q)));
$('count').textContent=`${list.length}／173张`;$('content').innerHTML=list.map(c=>{const vs=q==='native'?[c.variants[0]]:q==='all'?c.variants:c.variants.filter(v=>v.quality===q);return `<article class="${c.status==='已核对'?'':'candidate'}"><h2 class="quality-name" data-quality="${esc(vs.length===1?vs[0].quality:c.base_quality)}">${esc(c.name)}</h2><div class="meta">${esc(c.group)} · 原生${esc(c.base_quality)} · ${esc(c.status)}</div>`+vs.map(v=>`<section class="variant"><div class="cost">${esc(v.quality)} · ${esc(v.cost)}</div>${pair(v.compact,v.detail,v.target_heading)}${interactivePreview(c,v)}${c.branches.length?'<details><summary>该品质的全部阵赏分支</summary>'+c.branches.filter(b=>b.quality===v.quality).map(b=>`<div class="cost">${esc(b.branch)}阵赏</div>${pair(b.compact,b.detail,b.target_heading)}`).join('')+'</details>':''}</section>`).join('')+(c.common_rules.length?`<details><summary>机制核对资料</summary>${c.common_rules.map(t=>`<div class="rule">${copy(t)}</div>`).join('')}</details>`:'')+`<details><summary>取值与语义核对</summary><code>${esc(c.id)}</code><p>目标入口：${esc(c.target)}</p><p>${esc(c.source_rule)}</p>${c.note?`<div class="review-note">${esc(c.note)}</div>`:''}</details></article>`;}).join('')||'<p>没有匹配卡牌。</p>';}
...
report.update(pill_help_variants=419+36,pill_help_current_card_only=True,pill_help_no_duplicates=True,tooltip_modes=['compact','shift_detail','ctrl_pill_help'])
report.update(pill_help_click_toggle=True,pill_help_survives_keyup=True,pill_help_ignores_key_repeat=True,pill_help_max_characters=max(len(p['description']) for c in CARDS for v in c['variants'] for p in v['pill_descriptions']))
