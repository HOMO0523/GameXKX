...
对象标题独立于正文与pill说明。Ctrl说明按当前卡牌/品质收集；基础资源由资源栏介绍，蓄力与重箭合并解释，持续伤害共性只补一遍；按一下Ctrl开关，不因松开Ctrl关闭。交互检查执行原型的真实事件处理函数，验证Ctrl按下、长按不重复、松开、Shift返回及退出状态；它不等于浏览器视觉验证或UE运行验收。
