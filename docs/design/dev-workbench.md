...
正式游戏世界中，首次进入临时试验会先保存正常进度；保存失败则拒绝开始试验。纯2D入口在没有训练恢复点时读取正常默认存档，已有但不可读的存档不会被静默替换。游戏内确认退出前会保存，失败时保留退出确认供玩家处理。
...
功能以最终验收记录为准。普通 `GameXXK Shipping` 不创建开发子系统、不注册开发输入或文件处理。专用 `GameXXKDev Shipping` 使用 Shipping 优化并保留完整 F10、JSON 文件协议和临时会话存档保护；它需要独立的包内验证。Development 继续保留开发功能。原有玩法、伤害与装备规则仍由项目现有运行时负责。

运行 `powershell -File scripts/package_game.ps1 -Flavor ShippingF10` 生成带 F10 的 Shipping 试玩包；`-Flavor Development` 生成普通开发包。Shipping 试玩包继续排除仅由编辑器画质比较夹具使用的 4K 动画母图，保留正常玩法和 F10 战斗使用的 1K/2K 图集。原图和项目资产不删除。
