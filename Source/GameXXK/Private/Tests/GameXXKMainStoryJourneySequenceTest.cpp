...
bool FGameXXKMainStoryOutsideDepartureTest::RunTest(const FString&)
{
	...
	TestEqual(TEXT("action names the stage"),View.Options[0].Text.ToString(),FString(TEXT("进入1-1战斗")));
    ...
    TestEqual(TEXT("departure directly shows battle"),MVP->GetRuntimeState().Screen,EGameXXKScreen::Battle);
	...
	TestFalse(TEXT("active battle cannot be restarted from the task button"),Story->StartTask(TEXT("S00-04")));
	TestEqual(TEXT("same journey is retained"),MVP->GetRuntimeState().NarrativeProgress.MainStory.JourneyId,Journey);
	...
	TestFalse(TEXT("the direct battle must not enter its gate a second time"),Story->EnterJourneyGate(Gate));
...
bool FGameXXKMainStoryAtomicGateEntryTest::RunTest(const FString&)
{
	...
	TestFalse(TEXT("failed checkpoint does not publish battle entry"),Story->BeginTaskJourney());
	...
	TestEqual(TEXT("failed write keeps the player at the dialogue"),MVP->GetRuntimeState().Screen,Before.Screen);
	MVP->SetSaveSlotWriteDelegateForTest(FGameXXKSaveSlotWriteDelegate::CreateLambda([](USaveGame*,const FString&,int32){return true;}));
	TestTrue(TEXT("retry enters the same encounter"),Story->BeginTaskJourney());
...
bool FGameXXKMainStoryLegacyGateDialogueTest::RunTest(const FString&)
{
    ...
    if(!FGameXXKTrainingRules::StartChallenge(State.Training,TEXT("Training.Normal.1-1")) ||
       !FGameXXKMainStoryRules::GenerateDedicatedJourneyMap(State,TEXT("S00-04"),73431))return false;
...
bool FGameXXKMainStoryGateRealStorageTest::RunTest(const FString&)
{
	...
	const int32 Gate=MVP->GetRuntimeState().NarrativeProgress.MainStory.GateNodeIds.Array()[0];
	MVP->GetMutableRuntimeState().ReachableRouteNodeIds={Gate};
	const bool Entered=MVP->GetRuntimeState().CardRun.bHasActiveCardBattle;
...
bool FGameXXKMainStoryAftermathResumeTest::RunTest(const FString&)
{
	...
	const auto Journey=MVP->GetRuntimeState().NarrativeProgress.MainStory.JourneyId;
	const int32 Gate=MVP->GetRuntimeState().NarrativeProgress.MainStory.GateNodeIds.Array()[0];
	if(!MVP->GetRuntimeState().CardRun.bHasActiveCardBattle && !Story->EnterJourneyGate(Gate))return false;
	MVP->GetMutableRuntimeState().CardRun.ActiveBattle.Phase=EGameXXKCardBattlePhase::Victory;
	bool Cleared=false;FGameXXKTrainingReward Reward;
	...
	Story->PauseActivity();
	TestFalse(TEXT("task route already closed before aftermath"),MVP->GetRuntimeState().Training.bChallengeActive);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGameXXKStoryDirectBattleEntryTest,
    "GameXXK.MainStory.DepartureButtonDirectlyStartsSpecialBattle",EAutomationTestFlags::EditorContext|EAutomationTestFlags::EngineFilter)
bool FGameXXKStoryDirectBattleEntryTest::RunTest(const FString&)
{
    auto* Instance=NewObject<UGameInstance>();auto* MVP=NewObject<UGameXXKMVPSubsystem>(Instance);
    if(!MVP->StartGame())return false;
    auto* Story=NewObject<UGameXXKMainStorySubsystem>(Instance);Story->SetMVPForTest(MVP);
    if(!CompleteOutsidePrerequisites(MVP->GetMutableRuntimeState())||!Story->StartTask(TEXT("S00-04"))||!ReadPrelude(Story))return false;
    const auto ClearedBefore=MVP->GetRuntimeState().Training.ClearedStageIds;
    if(!TestTrue(TEXT("dialogue departure action succeeds"),Story->BeginTaskJourney()))return false;
    if(!TestTrue(TEXT("departure immediately starts the authored battle without a map click"),MVP->GetRuntimeState().CardRun.bHasActiveCardBattle))return false;
    TestEqual(TEXT("battle is the next visible surface"),MVP->GetRuntimeState().Screen,EGameXXKScreen::Battle);
    TestTrue(TEXT("dedicated task identity is retained"),FGameXXKMainStoryRules::IsDedicatedJourney(MVP->GetRuntimeState()));
    TestFalse(TEXT("no task-map overlay"),Story->WantsRouteOverlay());
    MVP->GetMutableRuntimeState().CardRun.ActiveBattle.Phase=EGameXXKCardBattlePhase::Victory;
    bool Cleared=false;FGameXXKTrainingReward Reward;
    if(!TestTrue(TEXT("victory resolves"),MVP->AdvanceTrainingChallengeEncounter(Cleared,Reward)))return false;
    TestEqual(TEXT("victory returns to workbench and authored aftermath, not a route map"),MVP->GetRuntimeState().Screen,EGameXXKScreen::Town);
    TestFalse(TEXT("special challenge closes after its single battle"),MVP->GetRuntimeState().Training.bChallengeActive);
    TestTrue(TEXT("authored aftermath remains available"),FGameXXKMainStoryRules::HasPendingAfterBattleDialogue(MVP->GetRuntimeState(),TEXT("S00-04")));
    TestFalse(TEXT("ordinary stage is not cleared"),Cleared);
    TestEqual(TEXT("ordinary clear count unchanged"),MVP->GetRuntimeState().Training.ClearedStageIds.Num(),ClearedBefore.Num());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGameXXKStoryRepeatedLegacyResumeTest,
    "GameXXK.MainStory.RepeatedLegacyWaitingStateResumesBattle",EAutomationTestFlags::EditorContext|EAutomationTestFlags::EngineFilter)
bool FGameXXKStoryRepeatedLegacyResumeTest::RunTest(const FString&)
{
    auto* Instance=NewObject<UGameInstance>();auto* MVP=NewObject<UGameXXKMVPSubsystem>(Instance);
    if(!MVP->StartGame())return false;
    auto* Story=NewObject<UGameXXKMainStorySubsystem>(Instance);Story->SetMVPForTest(MVP);
    ...
    auto& State=MVP->GetMutableRuntimeState();
    if(!FGameXXKTrainingRules::StartChallenge(State.Training,TEXT("Training.Normal.1-1")) ||
       !FGameXXKMainStoryRules::GenerateDedicatedJourneyMap(State,TEXT("S00-04"),51337))return false;
    const auto Legacy=State;
    Story->Tick(.1f);
    TestTrue(TEXT("first legacy load starts battle"),MVP->GetRuntimeState().CardRun.bHasActiveCardBattle);
    if(!MVP->CancelTrainingChallengeToWorkbench())return false;
    Story->Tick(.1f);
    MVP->GetMutableRuntimeState()=Legacy;
    Story->Tick(.1f);
    TestTrue(TEXT("the same saved revision may be resumed again after returning"),MVP->GetRuntimeState().CardRun.bHasActiveCardBattle);
    return true;
}
#endif
