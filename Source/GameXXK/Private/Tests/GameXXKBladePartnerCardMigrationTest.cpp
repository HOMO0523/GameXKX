...
namespace GameXXKBladePartnerCardMigrationTest
{
		...
		FString& OutError)
	{
		for (int32 SeedOffset = 0; SeedOffset < 4096; ++SeedOffset)
		{
			...
			if (!RecruitResult.Companion.UnlockedPersonalCardIds.Contains(RequiredCardId))
			{
				continue;
			}

			TArray<FName> Selection = { RequiredCardId };
			for (const FName CardId : RecruitResult.Companion.UnlockedPersonalCardIds)
			{
				if (Selection.Num() >= 5)
				{
					break;
...
bool FGameXXKBladePartnerCardMigrationTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(
		...
		14);
	TestEqual(TEXT("the current save version includes permanent NPC formation migration"), FGameXXKSaveMigration::CurrentSaveVersion, 30);
	FString Error;
