...
bool FGameXXKBattleBoardWidgetTest::RunTest(const FString& Parameters)
{
	...
	const FVector2D PartyUnitClickPosition(830.0f, 420.0f);
	const FVector2D MisprojectedPartyUnitPosition(260.0f, 640.0f);
	TestTrue(TEXT("battle board opens command menu for living party unit"), BattleWidget->OpenCommandMenuForPartyUnit(0, PartyUnitClickPosition, MisprojectedPartyUnitPosition));
	...
	const float CommandMenuGapFromClick = PartyUnitClickPosition.X - (CommandMenuAnchor.X + CommandMenuWidth);
	TestTrue(TEXT("battle board anchors command menu to the left of the clicked party unit"), CommandMenuAnchor.X < PartyUnitClickPosition.X);
	TestTrue(TEXT("battle board keeps command menu close to the clicked party unit instead of a misprojected actor origin"), CommandMenuGapFromClick >= 8.0f && CommandMenuGapFromClick <= 48.0f);
	TestTrue(TEXT("battle board keeps command menu vertically near the clicked party unit"), FMath::Abs(CommandMenuAnchor.Y - (PartyUnitClickPosition.Y - 120.0f)) <= 24.0f);
