...
bool FGameXXKSorcererPartnerAllCardTextTest::RunTest(const FString& Parameters)
{
	...
	const TArray<FExpectedCardText> Expected = {
		...
		{TEXT("Profession.Sorcerer.SheLingHuo"), TEXT("基础：回复当前内力的10%，向上取整；仅实际溢出按100%转为护甲"), TEXT(""), TEXT("阵赏：执行标准寒冰伤害；回复1点气力，抽1张牌")},
		{TEXT("Profession.Sorcerer.FenMaiFu"), TEXT("基础：本场内力上限+4，当前内力不变；回复当前内力的10%"), TEXT(""), TEXT("阵赏：执行标准寒冰伤害；自身内力上限再+8并补满内力")},
		{TEXT("Profession.Sorcerer.LingYanLianDan"), TEXT("基础：护甲为0时，回复当前内力的10%，向上取整；仅实际溢出按100%转为护甲；已有护甲时翻倍"), TEXT(""), TEXT("阵赏：标准冰爆；全体友方各获得本次消耗护甲的25%，向下取整")},
		{TEXT("Profession.Sorcerer.HuLingMu"), TEXT("基础：回复当前内力的10%，向上取整；仅实际溢出按100%转为护甲"), TEXT(""), TEXT("阵赏：执行标准寒冰伤害；敌方全体获得2层虚弱")},
