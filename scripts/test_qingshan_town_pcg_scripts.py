...
class QingshanTownPCGScriptsTests(unittest.TestCase):
    ...
    def test_mutations_are_restricted_to_managed_graph_and_prototype_map_roots(self):
        ...
        self.assertEqual(implementation.count("if (!IsManagedGraphPath(GraphAssetPath))"), 2)
        self.assertIn("if (!IsManagedPrototypeMapPath(WorldPackageName))", implementation)
        self.assertIn("if (!IsManagedPrototypeMapPath(CurrentLevelPackageName))", implementation)
