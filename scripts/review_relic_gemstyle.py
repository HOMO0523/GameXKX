...
def main():
    ...
    draw.text((20, 16), f"遗物图标 · 宝石同系列画风 · {len(selected)}/45", font=title_font, fill=ink)
    size_draw.text((20, 16), f"64px / 48px 与灰度 · {len(selected)}/45", font=title_font, fill=ink)
    ...
    review.save(out / "relic-overview.png")
    sizes.save(out / "relic-small-sizes.png")
    print(json.dumps({"completedArt": len(selected), "expectedArt": 45,
                      "overview": str(out / "relic-overview.png"), "sizes": str(out / "relic-small-sizes.png")}, ensure_ascii=False))
