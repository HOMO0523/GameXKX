"""Transient driver: chunked batch import (restore 4K or 2K rollout) with GC between chunks."""
...
def main() -> int:
    mode = sys.argv[1] if len(sys.argv) > 1 else "two-k"
    ...
    out = {"mode": mode, "chunks": []}
    offset = 0
    while offset < TOTAL:
        argv = [f"--{mode}", "--offset", str(offset), "--limit", str(CHUNK)]
        ...
        print(f"[{mode}] offset={offset} ok={chunk_ok} imported={imported}", flush=True)
