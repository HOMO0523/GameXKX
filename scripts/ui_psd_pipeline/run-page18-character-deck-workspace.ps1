param(
    ...
    [string]$Backup = 'outputs/UI_PSD/Candidates/Backups/GameXXK_UI_Master_V1.before-page18-character-deck-workspace.psd',
    [string]$Report = 'outputs/UI_PSD/Candidates/GameXXK_UI_Master_V1.page18-character-deck-workspace.report.json',
    [string]$Validation = 'outputs/UI_PSD/Candidates/GameXXK_UI_Master_V1.page18-character-deck-workspace.validation.json',
    [string]$ExportDirectory = 'SourceArt/UI/PSD/gamexxk-v4/ui-master/Review/ManualEditing/Page18CharacterDeckWorkspace'
)
...
$builderPath = Resolve-InputFile (Join-Path $PSScriptRoot 'build-page18-character-deck-workspace-jsx.js') 'page-18 workspace builder'
...
Assert-New $reportPath 'page-18 workspace report'
Assert-New $validationPath 'page-18 workspace validation'
if (Test-Path -LiteralPath $exportDirectoryPath) {
    if (@(Get-ChildItem -LiteralPath $exportDirectoryPath -File).Count -gt 0) { throw "Page-18 export directory is not empty: $exportDirectoryPath" }
} else {
...
Assert-New $backupPath 'page-18 workspace backup'
...
if ($beforeHash -ne $backupHash) { throw 'Page-18 workspace backup hash mismatch' }

$jsxDirectory = Join-Path $projectRoot 'tmp/ui_psd_pipeline'
[IO.Directory]::CreateDirectory($jsxDirectory) | Out-Null
$jsxPath = Join-Path $jsxDirectory 'apply-page18-character-deck-workspace.jsx'
...
if ($LASTEXITCODE -ne 0) { throw "Page-18 workspace JSX generation failed: $LASTEXITCODE" }
...
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    ...
    if (Test-Path -LiteralPath $errorReceipt) { throw "Photoshop page-18 duplication failed: $(Get-Content -Raw -Encoding UTF8 $errorReceipt)" }
    throw "Photoshop did not write page-18 workspace report: $reportPath"
}

$reportData = [IO.File]::ReadAllText($reportPath, [Text.Encoding]::UTF8) | ConvertFrom-Json
if ($reportData.status -ne 'PASS' -or [int]$reportData.afterTopLevelCount -ne ([int]$reportData.beforeTopLevelCount + 1)) {
    throw 'Unexpected page-18 workspace report'
}
...
foreach ($record in @($reportData.exports)) {
    ...
    if (-not (Test-Path -LiteralPath $exportPath -PathType Leaf)) { throw "Missing page-18 workspace export: $exportPath" }
    $size = Get-PngSize $exportPath
    if ($size[0] -ne 1920 -or $size[1] -ne 1080) { throw "Unexpected page-18 workspace export dimensions: $exportPath" }
    $exports += [ordered]@{
...
if (-not [bool]$pixelResult.pixel_identical) { throw 'New page 18 is not pixel-identical to source page 03' }

$afterHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $psdPath).Hash.ToLowerInvariant()
if ($afterHash -eq $beforeHash) { throw 'PSD hash did not change after page-18 duplication' }
...
[ordered]@{
    ...
    translation = @($reportData.translation)
    beforeTopLevelCount = [int]$reportData.beforeTopLevelCount
    afterTopLevelCount = [int]$reportData.afterTopLevelCount
    pixelIdenticalToPage03 = [bool]$pixelResult.pixel_identical
    exports = $exports
} | ConvertTo-Json -Depth 8 | Set-Content -Encoding UTF8 -LiteralPath $validationPath

Write-Output 'Page-18 character-deck workspace PASS'
