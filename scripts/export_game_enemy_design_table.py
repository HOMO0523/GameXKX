...
def base_enemies():
    rows=[]
    for raw in calls(CATALOG.read_text(encoding="utf-8"),"Result.Add(Enemy("):
        a=split_top(raw)
...
def build(path):
    ...
    add_table(wb,"00_说明",["项目","内容"],[
        ["权威口径","2026-09-03批准重平衡规格；运行时目录、历练编制与本表按同一口径验证"],
        ...
        ["实现状态","v36已实装；351个难度/阶段意图预演、189组固定编制、阶段切换与存档迁移均有自动化证据"]],{"项目":25,"内容":120})
    enemy_rows=[]
    for e in d["enemies"]:
    ...
    add_table(wb,"10_实现状态",["项目","当前实现","验证口径"],[
        ["训练等级","固定5..135；战斗和游历均读取关卡CombatLevel","地狱3-1=125；地狱3-3=135"],["难度上下文","显式Normal/Hard/Hell并校验100/125/150%","预演与执行读取同一枚举"],["阶段模型","九名精英/首领按难度使用1/2/3阶段","阶段牌组连续且旧Intents为空"],["阶段转换","伤害包截断至1HP、满血新阶段、清负面、保正面与护甲","直伤、多段、DOT、反击共用规则"],["编制","27关×7=189组固定左中右与开场意图","挑战与游历读取同一EnemySlots"],["意图数值","36普通、42阶段一、39阶段二、39阶段三","108+126+78+39=351预演通过"],["地狱3-1关底","125级 秃鹫-白猿-巨蟾，阶段1/3/1","总原始阶段生命11178"]],{"当前实现":76,"验证口径":90})
