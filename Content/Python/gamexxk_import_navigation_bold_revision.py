"""Import only the requested warehouse/formation bold-ink art revisions."""
import hashlib
import json
...
for entry in manifest['images']:
    ...
    backup = out / ('T_TrainingNav' + entry['kind'] + '-before-bold.uasset')
    ...
    export.filename = str(out / (entry['kind'] + '-bold-runtime-export.png'))
...
(out / 'bold-import-report.json').write_text(json.dumps(reports, ensure_ascii=False, indent=2), encoding='utf-8')
