...
root=Path(unreal.Paths.project_dir());dest='/Game/GameXXK/UI/Battle/VFX/UltimateLightning057'
for i in range(4,8):
    name=f'T_UltimateLightning057_Atlas_{i:02d}'
    task=unreal.AssetImportTask();task.filename=str(root/'SourceArt/UI/Battle/VFX/UltimateLightning057UnifiedV2/Atlases'/f'{name}.png');task.destination_name=name+'_Unified';task.destination_path=dest;task.automated=True;task.replace_existing=True;task.save=True;task.factory=unreal.TextureFactory()
    unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([task]);t=unreal.load_asset(dest+'/'+task.destination_name)
